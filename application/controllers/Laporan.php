...
class Laporan extends CI_Controller {
	...
	public function index(){
		$data['title'] = 'Laporan';
		$data['la'] = $this->db->query("SELECT * FROM tbl_laporan RIGHT JOIN tbl_masuk ON tbl_laporan.kd_masuk = tbl_masuk.kd_masuk ")->result_array();
		$data['bulan'] = $this->db->query("SELECT DISTINCT DATE_FORMAT(tgl_jam_keluar,'%M %Y') AS bulan FROM tbl_keluar")->result_array();
		$this->load->view('laporan', $data);
	}

	function get_kod(){
		$q = $this->db->query("SELECT MAX(RIGHT(kd_masuk,3)) AS kd_max FROM tbl_keluar");
		$kd = "";
		if($q->num_rows()>0){
